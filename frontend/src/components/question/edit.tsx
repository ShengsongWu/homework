...
export const Edit: FC<IProps> = (props) => {
  ...
  const [questionType, setQuestionType] = useState(type ?? QUESTION_TYPE[0].value);
        ...
        rules={[
          {
            validator: async (_, options) => {
              if ((!options || options.length < 1) && questionType !== 'checkbox') {
        ...
        {(fields, { add, remove }, { errors }) => (
          <>
            {fields.map((field, index) => {
              return (
                <Form.Item
                  {...(index === 0 ? FORM_ITEM_LAYOUT : FORM_ITEM_LAYOUT_WITHOUT_LABEL)}
                  ...
                  <Form.Item
                    ...
                    noStyle>
                    <Input placeholder="input an option" className={style['option-input']} />
                  </Form.Item>
                  {fields.length > 1 ? (
                    <MinusCircleOutlined
  ...
  return (
    <div className="f-bs">
      ...
      <div className={classNames('fc-sc', style.operation)}>
        <CheckOutlined className={style['operation-button']} title="Save" onClick={save} />
        <CloseOutlined
